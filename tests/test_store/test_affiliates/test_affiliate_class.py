...
def test_uid():
    ...
    assert affiliate_instance.uid is None, "affilite uid default not set correctly"
...
def test_is_active():
    ...
    assert not affiliate_instance.is_active, "affilite is active not being set correctly"
    with raises(TypeError):
...
def test_is_deleted():
    ...
    assert affiliate_instance.is_deleted, "affilite is deleted not being set correctly"
    with raises(TypeError):
