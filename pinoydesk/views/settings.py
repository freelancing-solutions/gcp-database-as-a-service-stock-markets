...
class ExchangeDataView:
    ...
    def add_complete_stock_tickers_list(self, exchange_id: str, tickers_list: list) -> tuple:
        with self.client.context():
            ...
            exchange_list: list = ExchangeDataModel.query(ExchangeDataModel.exchange_id == exchange_id).fetch()
    ...
    def get_exchange_tickers(self, exchange_id: str) -> tuple:
        with self.client.context():
            ...
            if len(exchange_list) > 0:
                exchange_instance: ExchangeDataModel = exchange_list[0]
                tickers_list: list = exchange_instance.exchange_tickers_list
                return jsonify({'status': False, 'message': 'successfully obtained exchange tickers',
    ...
    def delete_exchange(self, exchange_id: str) -> tuple:
        ...
        exchange_list: list = ExchangeDataModel.query(ExchangeDataModel.exchange_id == exchange_id).fetch()
        if len(exchange_list) > 0:
            exchange_instance = exchange_list[0]
